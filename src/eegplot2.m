...
%                  button is pressed (see Outputs); called from pop_eegplot2() or 
...
%                  pop_eegplot2(), the channel labels of the dataset will be used. 
...
%
% See also: eeg_multieegplot2(), eegplot2event(), eegplot2trial(), eeglab()
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
   ...
   try, g.limits;		    catch, g.limits	    = [0 1000*(size(data,2)/2-1)/g.srate]; end;
...
%   set(u,'Units','Normalized')
  ...
  commandzoom = [ 'tmpstr = get(gcbf, ''windowbuttondownfcn'');' ...
                  'set(gcbf, ''windowbuttondownfcn'', [ tmpstr ''; eegplot2(''''zoom'''', gcbf, 1);'' ]);' ...
...
%         hold on
%        plot(data(g.chans-i+1,(lowlim+g.frames):(highlim+g.frames)) -meandata(g.chans-i+1)+i*g.spacing, ...
%             'color', g.color{mod(i-1,length(g.color))+1}, 'clipping','on')
        plot(data(g.chans-i+1,lowlim:highlim) -meandata(g.chans-i+1)+i*g.spacing, ...
             'color', 'b', 'clipping','on')
         hold on
